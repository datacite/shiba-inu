...
describe "Double click filter for sample" do
...
describe "Double click filter for long response" do
  let(:response)  { parse_reponse fixture_path + "reponses/user_agent_long_response"}
  let(:message)   {response.sample}
  ...
  context "Multiple messages" do
    ...
    it "adds correct access-method" do
      messages = response
      file1 = messages.select {|m| m["doi"] == "10.5438/0000-00SS" }.first
      file2 = messages.select {|m| m["doi"] == "10.6084/M9.FIGSHARE.3496814" }.first    
      expect(file1["access_method"]).to eq("robot")
      expect(file2["access_method"]).to eq("machine")
    end
  end
